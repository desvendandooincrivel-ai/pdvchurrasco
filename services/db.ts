...
import { User, InventoryItem, Product, Sale, CashRegister, AuditLog, InventoryMovement, NFe } from '../types';
...
export class AppDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(3).stores({
      users: 'id, name, role',
      inventory: 'id, name',
      products: 'id, name, category, type, barcode',
      sales: 'id, timestamp, cashRegisterId',
      cashRegisters: 'id, openingTime, status',
      auditLogs: 'id, timestamp, userId, action',
      movements: 'id, itemId, type, timestamp, userId',
